...
const App: React.FC<TokenBankProps> = () => {
    ...
    // TODO: 更换为实际部署的合约地址
    const TOKEN_BANK_ADDRESS = "0x1e1C756C1549e231C310098A89FcAE426542D0c5" as `0x${string}`; // TokenBank合约地址
    const TOKEN_ADDRESS = "0x1099454e0CE6E28Be836470982CAF2bfc88E3a72" as `0x${string}`; // ERC20代币合约地址 (正确的MyToken地址)
    ...
    // 在useEffect中加载ABI - 使用ref防止严格模式重复执行
    useEffect(() => {
        // 强制防护：如果已经尝试过加载，直接返回
        ...
        try {
            ...
            const tokenabi = loadABIFromSrc('MyToken');
            console.log('✅ MyToken ABI loaded:', tokenabi.length, 'items');
    ...
    useEffect(() => {
        if (abiLoaded && tokenBankABI && tokenABI && isConnected && userAddress) {
            // 强制防护：如果已经尝试过查询，直接返回
